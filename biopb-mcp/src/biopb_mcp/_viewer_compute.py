...
building a ``Client``) so its heavy ``da`` computes run in parallel. The viewer, however, scrubs planes **one at a time**
...
the default path — load-bearing once a cell has built a ``Client``.
