...
The deterministic layers of ``docs/skill-testing.md``: no agent, no session, no
...
beside the website that used to publish them — see §9 of that doc.
...
on the author's path, where a missing section or a bad ``requires:`` token
should stop the PR.
"""
