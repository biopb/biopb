...
`requires:`/`suggests:` grammar the agent resolves at runtime, the rule that a
suggested package has its absence answered in step 1, `[[wiki-links]]` landing on a skill that
...
cell rejects the skill**, and a `suggests:` package is judged exactly like a
`requires:` one here: being optional to the skill does not make the downgrade
optional. A shipped catalog goes to every platform, so a skill that only resolves
...
job at about a second each — where the CI matrix screens five. A **required**
package must resolve on all nine; a **suggested** one may have a hole, because
that is what the body's degraded path is for, but must resolve somewhere.
