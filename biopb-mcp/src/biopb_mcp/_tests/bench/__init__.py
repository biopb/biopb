...
does not import the store, glob `_docs_data`, or know which docs ship, so
promoting or banking a doc changes the seed and no file in this tree. What a run measured is recorded in `session.json`, and which two sessions
...
**It tests what we ship, twice over.** A procedure body arrives through the
real :mod:`biopb_mcp.mcp._docs` — the index in the handshake and ``read_doc``,
the same calls the runtime makes — and every run happens against a real shim-spawned
