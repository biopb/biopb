"""Do the docs actually reach the wheel?

Every other test in this suite reads the checkout, so all of them pass whether
or not the docs ship. If they stop shipping, imports still work, this suite is
still green, and the agent is handed an index of nothing. The runtime
deliberately cannot catch that -- it is on the agent's path and must degrade
rather than raise (`_docs.shipped_ids` leaves a breadcrumb, nothing more) -- so
it is caught here, by building the thing that ships and looking inside it.
...
`[tool.setuptools.package-data]` glob. Breaking one is harmless -- which is why
this asserts the *outcome* rather than either mechanism. The glob is therefore
redundant today and worth keeping anyway: it is what survives if
include-package-data is ever turned off.

Everything under the directory ships, banked docs included: what a session is
*offered* is the seed index's decision, and a file the index does not list is
already invisible. `test_seed.py` holds that end.
...
from .conftest import DOCS_DIR, shipped_files

# biopb-mcp/ -- the package root, four levels up from _tests/docs/.
...
PREFIX = "biopb_mcp/mcp/_docs_data/"
...
def _shipped_md(wheel: zipfile.ZipFile) -> set[str]:
    return {
        ...
        if n.startswith(PREFIX) and n.lower().endswith(".md")
    }


def test_every_doc_in_the_tree_is_in_the_wheel(wheel):
    on_disk = {p.relative_to(DOCS_DIR).as_posix() for p in shipped_files()}
    assert on_disk, f"no docs at {DOCS_DIR}"
    assert _shipped_md(wheel) == on_disk


def test_the_seed_index_ships(wheel):
    """Without it a fresh install has nothing to seed from and starts blank."""
    assert "index.md" in _shipped_md(wheel)


def test_a_shipped_doc_is_not_empty(wheel):
    # A glob can match the names while something upstream truncates the content.
    for name in sorted(_shipped_md(wheel)):
        assert wheel.read(PREFIX + name).strip(), f"{name} ships empty"
