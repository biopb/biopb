...
from .conftest import declared_packages, doc_id_of, offered_files
...
# Which doc's claims each package below is asserted on behalf of. Adding a
# package-tier doc without adding an entry fails the coverage test.
...
def _third_party_requirements(path) -> dict[str, SpecifierSet]:
    """`{name: specifier}` for one doc's `packages:` entries, workspace aside.

    Each is already a PEP 508 requirement (`name`, `name>=X`, `name~=X`), so it
    is parsed rather than split on operators.
    """
...
def declared() -> dict[str, dict[str, SpecifierSet]]:
    """`{doc_id: {package: specifier}}` across the shipped store."""
    return {doc_id_of(p): _third_party_requirements(p) for p in offered_files()}


def test_every_declared_package_is_covered_here(declared):
    """The guard against this layer going unmanned again. A doc that declares a
    third-party package with nothing asserting its surface is a doc nobody
    checked against the library it drives."""
    missing = [
        f"{doc_id}: {sorted(uncovered)}"
        for doc_id, packages in declared.items()
        if (uncovered := set(packages) - COVERED.get(doc_id, set()))
    ]
    assert not missing, (
        "shipped docs declare packages with no contract test:\n"
        + "\n".join(missing)
        + "\n\nAdd assertions and list the package in COVERED."
    )


def test_covered_is_not_stale(declared):
    """The other direction: a COVERED entry naming a doc or a package that no
    longer ships is a table to update, not a failure to debug."""
    stale = [
        f"{doc_id}: {sorted(pkgs - set(declared.get(doc_id, {})))}"
        for doc_id, pkgs in COVERED.items()
        if pkgs - set(declared.get(doc_id, {}))
    ]
    assert not stale, "COVERED names what no doc declares:\n" + "\n".join(stale)


def test_every_installed_declared_package_actually_imports(declared):
    ...
    needs no hand-written assertion per package: it runs over whatever the store
    declares. Conditioned on the distribution being *installed* rather than on
    which env this is -- doc_contracts.py gives each package its own, so
    elsewhere the others are legitimately absent, and unimportable-when-present
    is fatal on every platform either way.
    """
        ...
        for name in packages:
            ...
            for module in top_level or [name.replace("-", "_")]:
                ...
                # Not just ImportError: a package can raise anything at import.
                    ...
                    pytest.fail(
                        f"{doc_id} declares {name}, which is installed "
                        f"({dist_version(name)}) and does not import:\n"
                        f"  import {module} -> {type(exc).__name__}: {exc}\n"
                        "A declared package the agent cannot import is a doc that "
                        "dead-ends at step 1 on every platform."
                    )


def test_the_installed_version_is_inside_every_declared_range(declared):
    ...
    for doc_id, packages in declared.items():
        for name, spec in packages.items():
            ...
            try:
                ...
                # Each env carries one declared package (doc_contracts.py), so
                # the others are legitimately absent here rather than missing.
                continue
            assert spec.contains(installed), (
                f"{doc_id} declares {name}{spec}, installed is {installed} -- "
