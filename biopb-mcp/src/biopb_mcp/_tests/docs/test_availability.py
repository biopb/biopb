...
def _declared(directory: Path = DOCS_DIR) -> list[tuple[str, str]]:
    """`(doc_id, requirement)` for every third-party `packages:` entry."""
    out = []
    for path in offered_files(directory):
        for spec in declared_packages(path):
            if Requirement(spec).name.lower() not in _WORKSPACE_DISTS:
                out.append((doc_id_of(path, directory), spec))
...
def test_the_extractor_finds_third_party_packages(docs_dir):
