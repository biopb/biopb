...
1. **Check the requirements** *(blocking)*. Resolve the Requirements line against
   `server_status`; [[kernel]] covers what to do about a gap. Read the
...
| More tracks than there were ever cells, and the number grows through the movie | Two causes, one symptom: counting `track_id`, which ends at every division, where lineage was wanted; and `splitting_cutoff` left at its default `False`, so each daughter starts an unrelated family | Both — count `tree_id`, *and* set `splitting_cutoff`. Measured: 161 / 97 / 67 lineages for 65 founders with `track_id` / splitting off / the full method |
| The derived cutoff was rounded up "to be safe", and nothing in the output looks wrong | A cutoff above what the biology allows links each object to a neighbour. Observed: an ablation arm derived 15 px from the user's own speed prior, padded it to 20 px as a "conservative allowance", and posted the worst false-link rate of the passing arms | Step 6's largest-step-in-µm/min. The truncation check cannot see this one — only 0.3% of links sit near the cutoff, while speed runs +22% at 3× the derived value |
