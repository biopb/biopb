...
temporary name in the kernel. Read [[napari-viewer]] before pulling pixels off data
sources.
...
5. **Visual check** *(non-blocking)*. Put the disagreement on screen, not just
   ...
   Show truth and prediction together — `blending="additive"` on [[napari-viewer]], or
