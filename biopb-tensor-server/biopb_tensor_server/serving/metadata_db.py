...
class MetadataDatabase:
    ...
    def sync_source_added(self, source_id: str, adapter: SourceAdapter) -> None:
        ...
        if report is not None and metadata and "rois" in metadata:
            ...
            # Annotations are off, so nothing above read the ROIs and the
            # whole `rois` key stays -- but a Mask's `bin_data` is arbitrary
            # binary, unlike every other shape kind, and must never reach
            # this SQL-queryable column regardless of whether annotation
            # import ran (biopb/biopb#1059 step 4, "Rasterizing OME masks").
