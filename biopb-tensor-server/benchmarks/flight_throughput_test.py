...
class MockChunkServer(flight.FlightServerBase):
    ...
    def do_get(self, context, ticket):
        return flight.RecordBatchStream(
            pa.RecordBatchReader.from_batches(self._batch.schema, [self._batch])
        )


def bench_flight_ceiling(
    port: int, batch: pa.RecordBatch, concurrency: int, iterations: int
):
...
class TestFlightCeiling:
    ...
    def test_ceiling_4mb(self, batch_4mb, concurrency):
        ...
        if concurrency >= 4:
            assert result["throughput_mb_s"] > 3000, (
                f"Expected >3000 MB/s ceiling, got {result['throughput_mb_s']:.0f}"
            )
...
class TestRealServerThroughput:
    ...
    def test_cached_reads_4mb(self, batch_4mb, concurrency):
        ...
        result = bench_real_server(
            8917, batch_4mb, concurrency, iterations=20, use_file_cache=True
        )
        print(
        ...
        if concurrency >= 4:
            assert result["throughput_mb_s"] > 2500, (
                f"Expected >2500 MB/s for cached reads, got {result['throughput_mb_s']:.0f}"
            )
    ...
    def test_raw_reads_4mb(self, batch_4mb, concurrency):
        ...
        result = bench_real_server(
            8918, batch_4mb, concurrency, iterations=20, use_file_cache=False
        )
        print(
        ...
        assert result["throughput_mb_s"] > 500, (
            f"Expected >500 MB/s for raw reads, got {result['throughput_mb_s']:.0f}"
        )
