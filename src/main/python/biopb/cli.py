...
def _resolve_mode(config: Path, remote: bool, token: Optional[str]) -> Optional[str]:
    ...
    if remote:
        ...
        if token:
            # Validate here with the shared rule the tensor `launch` applies, so
            # the two layers can't disagree: an invalid --token that this layer
            # accepted would be silently regenerated by `launch`, leaving the
