...
class TestProcessImageLazy:
    ...
    def test_tensor_server_url_env_uses_external_server(self):
        """TENSOR_SERVER_URL should upload lazy data to an external writable tensor server."""
        import subprocess
        import tempfile
        import shutil
        import threading
        ...
        image_cache_dir = tempfile.mkdtemp(prefix="biopb-test-env-loc-")
        tensor_cache_dir = tempfile.mkdtemp(prefix="biopb-test-env-tensor-")
        external_location = "grpc://127.0.0.1:8820"

        CacheManager.reset()
        CacheManager.initialize(
            CacheConfig(
                backend="file",
                file_cache_dir=Path(tensor_cache_dir),
                file_max_total_bytes=512 * 1024 * 1024,
            )
        )
        tensor_server = TensorFlightServer(location=external_location, writable=True)
        tensor_thread = threading.Thread(target=tensor_server.serve, daemon=True)
        tensor_thread.start()

        proc = subprocess.Popen([
            "python", "-m", "biopb_image_base.mock_servicer",
            "--port", "50054",
            "--cache-dir", image_cache_dir,
            "--cache-size", "1GB",
        ], env={
            **os.environ,
        ...
        try:
            ...
            assert response.image_data.HasField("lazy_data")
            serialized = response.image_data.lazy_data
            assert serialized.location == external_location

            from biopb.tensor.client import TensorFlightClient

            client = TensorFlightClient(serialized.location)
            result = client.tensor_from_pb(serialized).compute()
            ...
            shutil.rmtree(tensor_cache_dir, ignore_errors=True)
